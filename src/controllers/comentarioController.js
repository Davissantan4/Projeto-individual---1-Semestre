...
function ultimasParticipacoes(req, res) {
    comentarioModel.ultimasParticipacoes().then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado);
        } else {
            res.status(204).send("Nenhum resultado encontrado!");
        ...
        console.log(erro);
        console.log("Houve um erro ao buscar as últimas participações: ", erro.sqlMessage);
        res.status(500).json(erro.sqlMessage);
    });
}
